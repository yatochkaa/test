...
async def get_search_tree(session, car_id: int) -> dict:
    """getSearchTree: carType ДОЛЖЕН быть числом (Integer), lang — число (16).
    Точный код типа ТС заранее неизвестен -> перебираем кандидатов и
    запоминаем сработавший в _WORKING_CAR_TYPE_ID."""
...
async def get_articles(session, car_id: int, str_id: int) -> dict:
    """getArticles: carType тоже число. Берём подтверждённый getSearchTree-тип,
...
def _flatten_tree(node: Any, acc: list) -> None:
    ...
    if isinstance(node, dict):
        sid = _to_int(_pick(node, "strId", "id", "nodeId", "assemblyGroupNodeId"))
        name = _pick(node, "name", "assemblyGroupName", "text", "title")
...
async def get_oem_articles(session, car_id: int, str_id: int) -> list:
    ...
    if not r.get("_ok"):
        return []
    out, seen = [], set()
    for it in _as_list(r["data"]):
        if not isinstance(it, dict):
            continue
        brand = _pick(it, "brandName", "brand", "manuName", "supplierName")
        art = _pick(it, "articleNumber", "article", "number", "oem")
